    ...
    public function alltickets(Object $request)
    {
       ...
       $allTickets = DB::table('tickets')
        ...
        ->when(isset($request->codiceFiscale), function ($query) use ($codiceFiscale) {
            return $query->where('tickets.client_id', $codiceFiscale[0], $codiceFiscale[1]);
        })
        ...
        ->when($nominativo, fn($q) =>
            $q->where('ticket_clients.full_name', 'like', "%{$nominativo}%")
                ->orWhere('ticket_clients.company_name', 'like', "%{$nominativo}%")
        )
